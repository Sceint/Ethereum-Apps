...
contract('Conference', function (accounts) {
      ...
      function (conference) {
        ...
        conference.buyTicket({
          ...
          function () {
            ...
            return conference.refundTicket(accounts[1], ticketPrice, {
              from: accounts[1]
            });
          }).then(function () {
  ...
  // 5th testcase
  it("buyMultipleTickets", function (done) {
    var c = Conference.at(Conference.address);
    Conference.new({
      from: accounts[0]
    }).then(
      function (conference) {
        ...
        conference.buyMultiple(5, {
          ...
          function (num) {
            assert.equal(num, 5, "there should be 5 registrant");
            return conference.registrantsPaid.call(accounts[1]);
          }).then(function (amount) {
  ...
  // 6th testcase
  it("Destory Contract(80% amount should go to speaker)", function (done) {
    var c = Conference.at(Conference.address);
    Conference.new(accounts[9], {
      from: accounts[0]
    }).then(
      function (conference) {
        var ticketPrice = web3.toWei(.05, 'ether');
        var initialBalance = web3.eth.getBalance(conference.address).toNumber();
        var speakerInitialBalance = web3.eth.getBalance(accounts[9]).toNumber();
        conference.buyMultiple(5, {
          from: accounts[1],
          value: ticketPrice * 5
        }).then(
          function () {
            conference.destroy({
              ...
              function () {
                var speakerNewBal = web3.eth.getBalance(accounts[9]).toNumber();
                var diff = speakerNewBal - speakerInitialBalance;
                assert.equal(diff, (5 * ticketPrice * 80) / 100, "Sender's paid but not listed");
                done();
              }).catch(done);
          }).catch(done);
      }).catch(done);
