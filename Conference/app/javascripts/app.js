...
window.App = {
		...
		Conference.deployed().then(function (instance) {
			...
			conference.changeCity(city, {
				...
				function (location) {
					...
					$("#changeCityResult").html(msgResult);
	...
	setRating: function (userAddress, userRating) {
		var self = this;
		Conference.deployed().then(function (instance) {
			conference = instance;
			conference.setRating(userRating, {
				from: userAddress
			}).then(
				function () {
					return conference.userRated.call(userAddress);
				}).then(
				function (rating) {
					var msgResult;
					if (rating == userRating) {
						...
						msgResult = "Already Rated or Not Registered";
					}
...
window.addEventListener('load', function () {
	...
	$("#onClickChangeCity").click(function () {
		var val = $("#changeCity").find(":selected").text();
		if (val == "Select")
			$("#changeCityResult").html("Select a City");
		else
			App.changeCity(val);
	});
	...
	$("#setRating").click(function () {
		var val = $("#rateYo").rateYo("rating");
		var userAddress = $("#userAddress").val();
		App.setRating(userAddress, val);
	});
